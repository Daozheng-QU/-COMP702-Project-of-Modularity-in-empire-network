...
class Empire_Graph(object):
    ...
    def __init__(self, G):
        ...
        for edge in self.Temp_G.edges():
            ...
            node_i = edge[0]
            node_j = edge[1]
            #If node_i is not in the dictionary(self.Node_Index), Id of storage node i
            if node_i not in self.Node_Index:
                self.Node_Index[node_i] = idxi
                idxi = idxi + 1
            #If node_j is not in the dictionary(self.Node_Index), Id of storage node j
...
# Print the result of community division under the maximum Q value
print(" the maximal Q is %.3f, division: %s" % (max_Q, division))